...
.PHONY: all
all: apigateway authservice database frontend imageservice postservice searchservice userservice
...
.PHONY: format
format:
	@gofmt -w $$(find apps/apigateway apps/postservice apps/searchservice -name '*.go' -not -path '*/genproto/*')
...
.PHONY: lint
lint:
	@test -z "$$(gofmt -l $$(find apps/apigateway apps/postservice apps/searchservice -name '*.go' -not -path '*/genproto/*'))"
	@find apps/authservice/src apps/userservice/src apps/imageservice/src \
