<script lang="ts">
  ...
  function collapseIfIdle(event: FocusEvent) {
    const container = event.currentTarget as HTMLElement;
    // Deferred: relatedTarget is unset when the expand button unmounts mid-click.
    setTimeout(() => {
      if (container.contains(document.activeElement)) return;
      if (!content.trim() && !imageBlob) expanded = false;
    });
  }
...
<GlassCard>
  <div class="card-body p-4 sm:p-5" onfocusout={collapseIfIdle}>
    {#if !expanded}
      <button
        type="button"
        class="flex w-full items-center gap-3 rounded-full bg-base-100/40 px-4 py-3 text-left transition-colors hover:bg-base-100/60 sm:gap-4 sm:py-3.5"
        onclick={expand}
        onfocus={expand}
      >
        <Avatar name={user?.name} size="md" photoKey={user?.profilePhotoKey} />
        <span class="font-serif text-base opacity-60 sm:text-lg"
