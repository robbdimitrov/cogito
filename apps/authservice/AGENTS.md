...
- Session expiry must be enforced on reads as well as periodic cleanup.
  Cleanup spanning replicas must be harmless when run concurrently.
- The service accepts only calls authenticated by the internal interceptor.
  The gateway owns end-user authorization for session listing and deletion;
  preserve that boundary and do not make these RPCs publicly reachable.
