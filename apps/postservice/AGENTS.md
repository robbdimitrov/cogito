...
- Enforce post ownership in the mutation query or transaction. In particular,
  deletion must constrain both post ID and user ID.
- Preserve atomic counters and relationship changes for likes, reposts,
  replies, and deletes. Avoid check-then-act sequences outside a transaction.
...
- Use parameterized SQL and map absence, conflicts, and invalid relationships
  to deliberate gRPC statuses rather than raw database errors.
