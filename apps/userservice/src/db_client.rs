    ...
    async fn get_user(
        ...
        let row = sqlx::query(
            r#"SELECT id, name, username, email, bio, profile_photo_key, cover_photo_key,
                (SELECT count(*)::int FROM posts WHERE user_id = users.id AND in_reply_to_id IS NULL) AS posts,
                (SELECT count(*)::int FROM posts WHERE user_id = users.id AND in_reply_to_id IS NOT NULL) AS replies,
        ...
        Ok(row.map(|r| User {
            ...
            replies: r.get::<'_, Option<i32>, _>("replies").unwrap_or(0),
