...
pub(crate) fn verify(provided: &str) -> bool {
    match TOKEN.get() {
        Some(expected) => ct_eq_str(provided, expected),
        None => false,
    }
}
...
#[cfg(test)]
pub(crate) fn init_for_test() -> &'static str {
    TOKEN.get_or_init(|| "test-internal-token".to_string())
}
