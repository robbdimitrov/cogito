...
| Search outbox | Writes to `search_outbox` table + `pg_notify`; searchservice worker polls and indexes to Meilisearch |
...
**postservice** — Enforce post-type exclusivity (reply–quote via application; repost via DB CHECK). Enforce ownership on delete. Compute all counters at read time. Extract and store hashtags. Write to `search_outbox` on post mutations.
...
**searchservice** — Consume `search_outbox` via LISTEN/NOTIFY. Sync entities to Meilisearch indexes (users, posts, hashtags). Run full backfill on startup under advisory lock. Serve SearchService gRPC.

## Protobuf Contract

- Defined in `pkg/pb/thoughts.proto`. 5 gRPC services.
- Go bindings generated by `make proto` into `genproto/` per service (apigateway, postservice, searchservice). Never edited manually.
