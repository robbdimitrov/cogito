...
**flowservice** — Consume Redpanda `entity-changes` and `activity` topics. Maintain notification records, materialize feed rows for regular accounts, expose NotificationService and SearchService gRPC, and keep the Meilisearch index in sync.

## Protobuf Contract

- Defined in `pkg/pb/cogito.proto`. 6 gRPC services.
- Go bindings generated by `make proto` into `genproto/` per service (apigateway, postservice). Never edited manually.
- Rust bindings generated per service via `build.rs` at compile time (authservice, userservice, imageservice, flowservice).
