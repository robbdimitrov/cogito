...
| eventsservice | 1 | 100m | 64Mi | 128Mi |
...
| eventsservice | TCP | 5050 | 5s / 10s | 10s / 15s |
...
| database-url | authservice, userservice, postservice, imageservice, searchservice, eventsservice (DATABASE_URL) |
...
| EVENTS_SERVICE_ADDR | eventsservice:5050 |
...
| VALKEY_URL | redis://dragonfly:6379 |
...
PostgreSQL must run with `wal_level=logical` so Redpanda Connect `pg_cdc` can relay `outbox` inserts.
...
| Redpanda | connect/eventsservice | /var/lib/redpanda/data | Kafka-compatible event log |
